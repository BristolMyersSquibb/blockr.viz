...
#   TRTA) -> drilldown table.  The table pushes its claim into
#   blockr.dm::new_drill_filter_block over the control channel (no link);
...
#   adsl pull.
#
# The drill filter resolves the claimed column to a dm table itself (AEDECOD
# for a term click, AESEV for a severity click), so EVERY section drills
# against ONE filter, and the sender never names a table.
#
...
board <- new_dock_board(
  blocks = c(
    ...
    tbl = new_table_block(ctrl_target = "bridge",
                          block_name = "AE by severity table"),
    bridge = blockr.dm::new_drill_filter_block(
  ...
  extensions = list(blockr.dag::new_dag_extension(),
                    blockr.viz::new_ctrl_bridge_extension())
)
