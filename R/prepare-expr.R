# ---------------------------------------------------------------------------
# COPIED WHOLE from blockr.extra (R/code-block-expr.R) on 2026-09-08.
#
# Why a copy and not a dependency: blockr.viz cannot import blockr.extra
# without dragging broom, mirai, DT and evaluate into every deploy, and the
# shared home both packages already have (blockr.ui) belongs to someone else.
# The chart block's `script` is an experiment used in roughly 1 chart block in
# 20; if it does not earn its keep the branch is deleted and this file goes
# with it. If it does, blockr.extra's code block switches to blockr.viz::cb_*
# and this stops being a copy.
#
# KEEP THE NAMES IDENTICAL to blockr.extra's, so that switch is a delete.
# ---------------------------------------------------------------------------

#' Compiling a prepare script into part of the chart block's expression
#'
...
#' @param slot Emit the `.(data)` bquote marker for the data name (`TRUE`, what
#'   a block expression wants), or leave the bare symbol alone (`FALSE`, what
#'   local evaluation wants). Added for the chart block, which needs the same
#'   compiled body in both forms: once to run the script on the frame it draws,
#'   once to put in its expression.
...
cb_expr <- function(parsed, specs, values = list(), data_name = "data",
  ...
  subs <- list()
  subs[[data_name]] <- if (slot) call(".", as.name(data_name)) else as.name(data_name)
  for (s in specs) {
