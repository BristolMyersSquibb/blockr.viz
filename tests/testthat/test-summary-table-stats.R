...
test_that("legacy presets map to catalog keys", {
  compact <- summary_table(mtcars, vars = "mpg", stats = "compact")
  mean_sd <- summary_table(mtcars, vars = "mpg", stats = "mean_sd")
  expect_identical(compact, mean_sd)

  expanded <- summary_table(mtcars, vars = "mpg", stats = "expanded")
  keys <- summary_table(
    mtcars, vars = "mpg",
...
test_that("a single stat key emits one un-indented row per variable", {
  out <- summary_table(mtcars, vars = "mpg", stats = "median_q1_q3")
...
test_that("several stat keys emit one indented row each", {
  out <- summary_table(mtcars, vars = "mpg", stats = c("n_pct", "mean_sd"))
...
test_that("selection is reordered to canonical catalog order", {
  fwd <- summary_table(mtcars, vars = "mpg", stats = c("n", "min_max"))
  rev <- summary_table(mtcars, vars = "mpg", stats = c("min_max", "n"))
...
test_that("n_pct is the share of non-missing rows per group", {
  ...
  out <- summary_table(df, vars = "x", by = "g", stats = "n_pct")
...
test_that("categorical and logical variables are unaffected by `stats`", {
  ...
  a <- summary_table(df, vars = c("sex", "flag"), stats = "mean_sd")
  b <- summary_table(df, vars = c("sex", "flag"), stats = c("n", "min_max"))
